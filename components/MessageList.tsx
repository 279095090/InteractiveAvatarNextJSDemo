...
export default function MessageList({ messages }: MessageListProps) {
    ...
    return <div ref={view} className="w-full border-none flex flex-col gap-1">
        {messages.reverse().map((message: Message) =>
            <div key={message.id} className="flex flex-row ">
                ...
                <span className="flex-1 backdrop-blur-sm bg-white/20 rounded-md text-sm p-1">{message.content}</span>
            </div>
        )}
    </div>
