...
export async function POST(req: Request) {
  ...
  const lang = req.headers.get("cur_language") ?? "zh_CN"; 
  ...
  return AssistantResponse(
    ...
    async ({ forwardStream }) => {
      const runStream = openai.beta.threads.runs.stream(threadId, {
        model: 'gpt-4o-mini',
        instructions: instructions.replace('$language', getLang(lang)),
        assistant_id:
